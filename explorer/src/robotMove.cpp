...
void robotMove::init(const char* shield_name){
  ...
  if(stringIsEqual(shield_name, 'arduino')){
    ...
    _shield = 1;
    _motor_pin = vecAlloc(sizeof(_SEEDSTUDIO_SHIELD_PINOUT));
    _motor_pin_size = sizeof(_SEEDSTUDIO_SHIELD_PINOUT) / sizeof(int);
    vecCopy(_SEEDSTUDIO_SHIELD_PINOUT, _motor_pin_size, _motor_pin);
    for(int i = 0; i < 8; i++){
      pinMode(_motor_pin[i], OUTPUT);
    }
  }
